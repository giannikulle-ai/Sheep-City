...
describe('parity: no deity intent moves the pin', () => {
  ...
  it('luna-day.test.ts, seed 11: the end-of-day hash is unchanged', () => {
    ...
    expect(hashState(s)).toBe('618be9dbc3bc2476'); // moved in #113: a card/authored start's own id now runs through `tell`'s `repeats` (chronicle.stats), which the hash covers even where notability itself is unmoved; was 72b482515856e681
  });

  it('sheep-day.test.ts, seed 71: the end-of-day hash is unchanged', () => {
    let s = createInitialState(71);
    for (let i = 0; i < 1800; i++) s = tick(s);
    expect(hashState(s)).toBe('7e4da67be5ec78ba'); // moved in #113: a card/authored start's own id now runs through `tell`'s `repeats` (chronicle.stats), which the hash covers even where notability itself is unmoved; was 8971628f989ca315
  });
});
