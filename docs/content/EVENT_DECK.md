...
This deck does not have a `size` field yet. Sizing every card, one line each, and rebalancing the
weights to decision 16's own targets (small most days, big a few times a month) is **#102**, and it
waits on the sim ticket **#101** that teaches the engine what `size` means (big only while
watched, small during catch-up too, nothing forced). This PR keeps the coverage work — the widened
`conditions` below, and the moved `stargazingNight` moment kind (see its own card, below) — and
drops the round-1 pace retune entirely: every card's `weight.base` this PR ships is back at
trunk's own value, except `merchantCaravan` (kept at 10, not trunk's 14 — its own card entry below
says why) and `windfall` (back at trunk's 5, unchanged from before this PR's earlier rounds other
than the data already being coins-free per decision 12).
...
and `windfall` the two named exceptions above) — they are what the coverage work alone, with no
size-aware rebalance, already gives the world over a month. #102 measures again once sizes and
their own weights land, against decision 16's actual targets (small on about four days in five,
big about three a farm month), which this table does not attempt.
