...
# All outputs land next to this script in tools/art/build/ (was /home/claude/v3out).
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "build")
# Chromium: set SHEEPCLIFF_CHROMIUM to a browser binary to use it; otherwise the pre-installed
# /opt/pw-browsers/chromium is used when present, else Playwright's own download.
CHROMIUM = os.environ.get("SHEEPCLIFF_CHROMIUM") or ("/opt/pw-browsers/chromium" if os.path.exists("/opt/pw-browsers/chromium") else None)
...
if __name__ == "__main__":
    ...
    with sync_playwright() as p:
        b = p.chromium.launch(**({"executable_path": CHROMIUM} if CHROMIUM else {})); pg = b.new_page(device_scale_factor=1)
    ...
    except OSError: font = ImageFont.load_default()   # labels only; never touches a sprite
    show = []
    ...
    for tname, oname in [("demo_template.html", "demo.html"), ("sim_template.html", "farm_sim.html")]:
        import os.path
        if not os.path.exists(os.path.join(HERE, tname)): continue   # templates live with the client, not the art lane
        demo = open(os.path.join(HERE, tname)).read()
    ...
    if os.path.exists(os.path.join(HERE, "demo_template.html")):
        demo = open(os.path.join(HERE, "demo_template.html")).read()
        demo = demo.replace("__META__", json.dumps(meta)).replace("__SHEET__", b64(sheet))
        for name, im in bgs.items(): demo = demo.replace(f"__BG_{name.upper()}__", b64(im))
        demo = demo.replace("__PHASES__", json.dumps(PHASES))
