...
if __name__ == "__main__":
    import os
    SVG = os.path.join(os.path.dirname(os.path.abspath(__file__)), "build", "svg")
    os.makedirs(SVG, exist_ok=True)
    open(os.path.join(SVG, "scene.svg"), "w").write(scene())
    for k, fn in ASSETS.items():
        open(os.path.join(SVG, f"{k}.svg"), "w").write(asset_svg(fn()))
    open(os.path.join(SVG, "barn.svg"), "w").write(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 240" width="240" height="240">{defs()}{barn()}</svg>')
    print("svgs written")
