...
1. Append to `events` in `events/authored.json`. It shares `hooks`, `storybook`, `moment`, `beat` with a card, but trades `conditions`/`weight`/`limits` for `trigger` (one of four shapes: `predicates` — a list like a card's `conditions`, plus its own `cooldownSimDays`; `simDate` — a `season` and a `dayOfSeason`, a fraction 0 up to (not including) 1 of however long that season turns out to be, since #83 the sim's own seasons no longer have a fixed day count; `stockThreshold` — a Ledger `on`/`op`/`value` plus `cooldownSimDays`; `realDate` — a fixed real calendar `month` and `day`, plus an optional `windowSimMinutes`, for something that recurs on the same real date every year regardless of the sim's season, like DL's birthday), `variables` (an open bag of named parameters only this event's own hooks and storybook read — never put a `comment` key in here; it would look like a phantom parameter), a required sibling `variablesComment` explaining what the bag holds and why, and `priorityOver` (card ids, or bare parameter names like `mood`/`weather`, this event outranks while it runs).
2. Pick the trigger shape that actually fits: a fixed real date is `realDate`; a fixed point in the sim's own season cycle is `simDate`; a real state to react to (a drought, a surplus) is `stockThreshold`; anything else that reads like a card's conditions but should not be drawn randomly is `predicates`. The season calendar itself (nominal length, drift, and each season's real-date anchor) lives in `balance/farm.json`'s `outsideRules.seasons.calendar` — see "How to add a balance number" above.
