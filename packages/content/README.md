...
1. Append to `events` in `events/authored.json`. It shares `hooks`, `storybook`, `moment`, `beat` with a card, but trades `conditions`/`weight`/`limits` for `trigger` (one of three shapes: `predicates` — a list like a card's `conditions`, plus its own `cooldownSimDays`; `simDate` — a `season` and a `dayOfSeason` 1 to 9; `stockThreshold` — a Ledger `on`/`op`/`value` plus `cooldownSimDays`), `variables` (an open bag of named parameters only this event's own hooks and storybook read), and `priorityOver` (card ids, or bare parameter names like `mood`/`weather`, this event outranks while it runs).
