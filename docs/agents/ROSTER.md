...
Sim merge order: #60 first (save v6), then #43 and #61 after merging trunk. #40 the engine starts as soon as #59 and #60 land; #63 after #60. Check-ins hourly during the sprint, back to four-hourly when it is merged. All seven sprint PRs are on trunk (615 sim tests green at 55f7e72). Follow-ups: #77 the invariant fence landed as PR #79 (merged by the owner 14:51 UTC while its Verifier findings were open; the remaining findings are #80, fix worker on Sonnet, branch `lane/sim/80-fence-round-2`); #76 manual-mode showers; #78 the catch-up bench drift. Started 14:45 UTC under the model rules: #40 the event engine on Opus (High-gate sim core, branch `lane/sim/40-event-engine`). Next, one sim worker at a time: #63 disposition of farm builds (Sonnet) after #40, then #45 social graph, #48 crows. Phase 2 is open since #62 is pinned.
